...
class SevenSegmentDisplay:
    ...
    def showDigit(self, i):
        ...
        shiftOut(dataPin, clockPin, self.MSBFIRST, self.num[i])#Output the figures and the highest level is transfered preferentially.
    ...
    def loop(self):
        while True:
            for i in range(0,len(num)):
                GPIO.output(latchPin,GPIO.LOW)
                shiftOut(dataPin, clockPin, self.MSBFIRST, num[i])#Output the figures and the highest level is transfered preferentially.
                GPIO.output(latchPin, GPIO.HIGH)
                time.sleep(0.5)
            for i in range(0,len(num)):
                GPIO.output(latchPin,GPIO.LOW)
                shiftOut(dataPin, clockPin, self.MSBFIRST, num[i]&0x7f)#Use "&0x7f"to display the decimal point.
                GPIO.output(latchPin, GPIO.HIGH)
                time.sleep(0.5)
